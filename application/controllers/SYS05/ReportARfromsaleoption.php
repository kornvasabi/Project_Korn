...
//BEE+
class ReportARfromsaleoption extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
		...
		$html.= "<script src='".base_url('public/js/SYS05/ReportARfromsaleoption.js')."'></script>";
		echo $html;
	}
	...
	function search(){
		...
		$sql = "
				...
				into #main
				from(
					select A.LOCAT, A.CONTNO, A.CUSCOD, B.SNAM+B.NAME1+' '+B.NAME2 as CUSNAME, A.SDATE, A.DUEDT, A.OPTPRC AS NPRICE, A.OPTPVT AS VATPRC, A.OPTPTOT AS TOTPRC 
					...
					union  
					select A.LOCAT, A.CONTNO, A.CUSCOD, B.SNAM+B.NAME1+' '+B.NAME2 as CUSNAME, A.SDATE, A.DUEDT, A.OPTPRC AS NPRICE, A.OPTPVT AS VATPRC, A.OPTPTOT AS TOTPRC  
					from {$this->MAuth->getdb('HAROPMST')} A
					left join {$this->MAuth->getdb('CUSTMAST')} B  on A.CUSCOD = B.CUSCOD 
					where (A.SDATE <= '".$ARDATES."')  AND A.OPTPTOT > 0 AND (A.OPTPTOT > A.SMPAY OR  (A.OPTPTOT = A.SMPAY AND A.LPAYDT > '".$ARDATES."'))
					".$cond."  
				)main	
		...
		$sql = "
				...
				from(
					...
					where A.FLAG != 'C' AND A.PAYFOR = '005' AND CONTNO in (select CONTNO from #main)
		...
		$report = "
			<div id='table-fixed-ReportARfromsaleoption2' class='col-sm-12' style='height:100%;width:100%;overflow:auto;'>
				<table id='table-ReportARfromsaleoption2' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='100%'>
					<thead>
						<tr>
							<th colspan=".($vat == 'showvat' ? '22' : '12')." style='font-size:12pt;border:0px;text-align:center;'>รายงานลูกหนี้คงเหลือจากการขายอุปกรณ์</th>
						</tr>
						<tr>
	...
	function pdf(){
		...
		$content = "
			<table class='wf' style='font-size:7.5pt;height:700px;border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
				<tbody>
					<tr>
						<th colspan=".($vat == 'showvat' ? '13' : '12')." style='font-size:10pt;'>รายงานลูกหนี้คงเหลือจากการขายอุปกรณ์</th>
					</tr>
					<tr>
