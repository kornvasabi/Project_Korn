...
//BEE+
class ReportARkangpay_x extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
				...
				<div class='col-sm-12 col-xs-12' style='height:100%;overflow:auto;font-size:10.5pt;'>					
					<div class='row' style='height:90%;'>
						<div class='col-sm-12 col-xs-12' style='background-color:#0067a5;border:5px solid white;height:75px;text-align:center;font-size:12pt;color:white;font-weight:bold;'>	
							<br>รายงานลูกหนี้ค้างชำระ x งวดแรก<br>
						</div>
						<div class='col-sm-10 col-xs-10 col-sm-offset-1'>	
							...
							<div class='col-sm-3 col-xs-3'>	
								<div class='form-group'>
									ค้างชำระ (งวดแรก)
									<input type='text' id='EXPFRM1' class='form-control input-sm' style='font-size:10.5pt' value='1'>
								</div>
							</div>
							<div class='col-sm-3 col-xs-3'>	
								<div class='form-group' >
									...
									<div class='col-sm-12 col-xs-12' style='border:0.1px dotted #d6d6d6;'>	
										<div class='col-sm-6 col-xs-6'>
											<div class='form-group'><br>
												<input type= 'radio' id='exp1' name='exp'> ขาดตั้งแต่งวดที่ 1
											</div>
										</div>
										<div class='col-sm-6 col-xs-6'>
											<div class='form-group'><br>
												<input type= 'radio' id='expX' name='exp' checked> งวดใดๆ ภายใน x งวด
											</div>
										</div>
									</div>
								</div>
		...
		$html.= "<script src='".base_url('public/js/SYS05/ReportARkangpay_x.js')."'></script>";
		echo $html;
	}
	...
	function search(){
		...
		if($exp == 'exp1'){
			if($EXPFRM1 != ""){
				$cond .= " AND A.EXP_FRM <= 1 AND (A.EXP_PRD = ".$EXPFRM1.") ";
			}	
		}else{
			if($EXPFRM1 != ""){
				$cond .= " AND A.EXP_FRM <= ".$EXPFRM1." AND (A.EXP_PRD = ".$EXPFRM1.") ";
			}
		}
		...
		$sql = "
				...
				from(
					...
					SDATE, TOTPRC, TOTDWN, TOTDWN-PAYDWN as KANGDWN, TOTPRC-SMPAY as TOTAR, EXP_AMT, TOT_UPAY, LPAYD, T_NOPAY, EXP_PRD, EXP_FRM, EXP_TO, 
					DATEDIFF(DAY,isnull(LPAYD,SDATE),GETDATE()) as LATED, ACTICOD, BILLCOLL, sumintamt-sumpayint AS KANGINT, tumb, aumpcod, provcod
					FROM ( 
						...
						) g on a.contno=g.contno and a.locat=g.locatpay  
						left outer join {$this->MAuth->getdb('CUSTADDR')} D on (A.CUSCOD = D.CUSCOD) AND D.ADDRNO = B.ADDRNO  
						where  A.TOTPRC > 0 AND (A.TOTPRC-A.SMPAY) > 0 and A.SDATE BETWEEN '".$FRMDATE."' and '".$TODATE."' ".$cond."
					) as X  
				)main
		";//echo $sql; 
		...
		$sql = "
				...
				TOT_UPAY, LPAYD, convert(nvarchar,LPAYD,112) as LPAYDS, T_NOPAY, EXP_PRD, EXP_FRM, EXP_TO, LATED, ACTICOD, BILLCOLL, tumb, aumpcod, provcod
				from #main
				order by ".$orderby."
		";//echo $sql; exit;
		$query = $this->db->query($sql);
		...
		$head = "<tr style='height:30px;'>
				...
				<th style='vertical-align:top;text-align:right;'>ค้างงวด<br>งวดทั้งหมด</th>
				<th style='vertical-align:top;text-align:right;'>ค่างวดๆละ<br>งวดที่ค้าง</th>
				<th style='vertical-align:top;text-align:center;'>ชำระล่าสุด<br>จากงวดที่</th>
				<th style='vertical-align:top;'>Billcoll<br>ขาดการติดต่อ</th>
				</tr>
		";
			...
			foreach($query->result() as $row){$i++;
				$html .= "
					<tr class='trow' seq=".$NRow.">
						...
						<td align='right'>".number_format($row->EXP_AMT,2)."<br>".number_format($row->T_NOPAY)."</td>
						<td align='right'>".number_format($row->TOT_UPAY)."<br>".number_format($row->EXP_PRD)."</td>
						<td align='center'>".$this->Convertdate(2,$row->LPAYDS)."<br>".number_format($row->EXP_FRM).'-'.number_format($row->EXP_TO)."</td>
						<td align='center'>".$row->BILLCOLL."<br>".number_format($row->LATED)."</td>
					</tr>
				";	
			}
		}
		...
		if($i>0){
			$html = "
				<div id='table-fixed-ReportARkangpay_x' class='col-sm-12' style='height:100%;width:100%;overflow:auto;font-size:8pt;'>
					<table id='table-ReportARkangpay_x' style='background-color:white;' class='col-sm-12 display table table-bordered' cellspacing='0' width='calc(100% - 1px)'>
						<thead>
						<tr style='height:40px;'>
							<th colspan='13' style='font-size:12pt;border:0px;text-align:center;'>รายงานลูกหนี้ค้างชำระ x งวดแรก</th>
						</tr>
		...
		$report = "
			<div id='table-fixed-ReportARkangpay_x2' class='col-sm-12' style='height:100%;width:100%;overflow:auto;'>
				<table id='table-ReportARkangpay_x2' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='100%'>
					<thead>
						<tr>
							<th colspan='22' style='font-size:12pt;border:0px;text-align:center;'>รายงานลูกหนี้ค้างชำระ x งวดแรก</th>
						</tr>
						<tr>
							<td colspan='22' style='border:0px;text-align:center;'>".$rpcond." ออกรายงาน ณ วันที่ ".$this->today('today')."</td>
						</tr>
						".$head2."
					</thead>	
					<tbody>
	...
	function pdf(){
		...
		$head = "
				<tr>
					...
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:right;'>ค้างงวด<br>งวดทั้งหมด</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:right;'>ค่างวดๆละ<br>งวดที่ค้าง</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:center;'>ชำระล่าสุด<br>จากงวดที่</th>
			...
			foreach($query->result() as $row){	
				$html .= "
					<tr class='trow' seq=".$No.">
						...
						<td style='width:70px;' align='right'>".number_format($row->EXP_AMT,2)."<br>".number_format($row->T_NOPAY)."</td>
						<td style='width:60px;' align='right'>".number_format($row->TOT_UPAY)."<br>".number_format($row->EXP_PRD)."</td>
						<td style='width:70px;' align='center'>".$this->Convertdate(2,$row->LPAYDS)."<br>".number_format($row->EXP_FRM).'-'.number_format($row->EXP_TO)."</td>
		...
		$content = "
			<table class='wf' style='font-size:7.5pt;height:700px;border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
				<tbody>
					<tr>
						<th colspan='14' style='font-size:10pt;'>รายงานลูกหนี้ค้างชำระ x งวดแรก</th>
					</tr>
					<tr>
