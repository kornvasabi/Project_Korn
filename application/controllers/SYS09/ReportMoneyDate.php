...
********************************************************/
class ReportMoneyDate extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
		...
		$html = "
			<div class='k_tab1' name='home' locat='{$this->sess['branch']}' cin='{$claim['m_insert']}' cup='{$claim['m_update']}' cdel='{$claim['m_delete']}' clev='{$claim['level']}' today='".$this->today('today')."' style='height:calc(100vh - 132px);overflow:auto;background-color:white;'>
				<div class='col-sm-12 col-xs-12' style='background-color:#0480E0;border:5px solid white;height:75px;text-align:center;font-size:12pt;color:white;font-weight:bold;'>	
					<br>รายงานสรุปการรับเงินตามวันที่บันทึก<br>
				</div>
					...
					<div class='col-sm-6 col-xs-6'>
						<div class='form-group'>
							...
							<div class='col-sm-12'>	
								<div class='form-group'>
									จากวันที่
									<input type='text' id='INPDT_F' class='form-control input-sm' data-provide='datepicker' data-date-language='th-th' value='".$this->today('today')."' styl='font-size:10.5pt;'>
								</div>
							</div>
							<div class='col-sm-12'>	
								<div class='form-group'>
									ถึงวันที่
									<input type='text' id='INPDT_T' class='form-control input-sm' data-provide='datepicker' data-date-language='th-th' value='".$this->today('today')."' styl='font-size:10.5pt;'>
								</div>
							</div>							
						</div>
					</div>
		...
		";
		$html .="<script src='".base_url('public/js/SYS09/ReportMoneyDate.js')."'></script>";
		echo $html;
	}
	function conditiontopdf(){
		$data 	= 	array();
		$data[] = 	urlencode(
			$_REQUEST["LOCATRECV"].'||'.$_REQUEST["INPDT_F"].'||'.$_REQUEST["INPDT_T"].'||'.
	...
	function pdf(){
		...
		$LOCATRECV	= $tx[0];
		$INPDT_F	= $this->Convertdate(1,$tx[1]);
		$INPDT_T	= $this->Convertdate(1,$tx[2]);
		$LOCATPAY	= $tx[3];
		$PAYTYP		= $tx[4];
		$CODE		= $tx[5];
		...
		";
		$conp       = $this->db->query($sql);
		$rowcp	    = $conp->row();
		$COMP_NM    = $rowcp->COMP_NM;
		
		$sql = "
			select LOCATNM from {$this->MAuth->getdb('INVLOCAT')}
		";
		$qlocat     = $this->db->query($sql);
		$rowlc      = $qlocat->row();
		$LOCATNM    = $rowlc->LOCATNM; 
		...
		if($query->row()){
			foreach($query->result() as $row){$i++;
				$sql2 = "
					...
					and A.INPDT between '".$INPDT_F."' and '".$INPDT_T."' 
		...
		if($i > 0){
			$html .="
				<tr class='trow' style='background-color:#ebebeb;'>
					<td width='40px'  height='40px'	align='center' style='border-top:0.1px solid black;vertical-align:top;border-bottom:0.1px solid black;' colspan='4'>รวมทั้งสิ้น</td>
		...
		if($i > 0){
			$header = "
				<table class='wf fs9' cellspacing='0' style='border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
					<tr>
						<th colspan='16' style='font-size:11pt;' align='center'>".$COMP_NM."<br>รายงานสรุปรายรับตามวันที่บันทึก</th>
					</tr>
					<tr>
						<th colspan='16' style='font-size:11pt;' align='center'>
							<br>สาขา</br>&nbsp;&nbsp;".$LOCATNM."&nbsp;&nbsp;
							<b>ชำระเงินเพื่อ บ/ช สาขา</b>&nbsp;&nbsp;".$LOCATPAY."&nbsp;&nbsp;
							<b>จากวันที่</b>&nbsp;&nbsp;".$this->Convertdate(2,$INPDT_F)."&nbsp;&nbsp;
							<b>ถึงวันที่</b>&nbsp;&nbsp;".$this->Convertdate(2,$INPDT_T)."&nbsp;&nbsp;
