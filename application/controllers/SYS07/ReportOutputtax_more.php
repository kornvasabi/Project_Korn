...
//BEE+
class ReportOutputtax_more extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
				...
				<div class='col-sm-12 col-xs-12' style='height:100%;overflow:auto;font-size:10.5pt;'>					
					<div class='row' style='height:90%;'>
						<div class='col-sm-12 col-xs-12 bg-info' style='border:5px solid white;height:75px;text-align:center;font-size:12pt;color:white;font-weight:bold;'>	
							<br>รายงานภาษีขาย (ยื่นเพิ่มเติม)<br>
						</div>
		...
		$html.= "<script src='".base_url('public/js/SYS07/ReportOutputtax_more.js')."'></script>";
		echo $html;
	}
	...
	function search(){
		...
		$sql = "
				...
				from(
					...
					NETAMT, VATAMT, TOTAMT, case when FLAG = 'C' then 'ยกเลิก' else '' end as CANCEL, convert(char,INPDT,112) as INPDT, 
					FLAG, TAXTYP
					from {$this->MAuth->getdb('TAXTRAN')} 
					where INPDT between '".$FRMDATE."' and '".$TODATE."' and TAXFLG = 'A' ".$cond."
				)txsale
		";//echo $sql; 
		$query = $this->db->query($sql);
		
		$sql = "
				select TAXDT, TAXNO, CUSNAME, CONTNO, STRNO, DESCP, VATRT, NETAMT, VATAMT, TOTAMT, INPDT, CANCEL
				from #txsale
				".$orderby."
		";//echo $sql; exit;
		$query = $this->db->query($sql);
		...
		$head = "<tr style='height:25px;background-color:#D3ECDC;'>
					...
					<th style='vertical-align:middle;taxt-align:center;'>วันที่รันเพิ่มเติม</th>
			...
			foreach($query->result() as $row){$i++;
				$html .= "
					<tr class='trow' seq=".$NRow.">
						...
						<td align='center'>".$this->Convertdate(2,$row->INPDT)."</td>
			...
			foreach($query->result() as $row){
				$report .= "
					<tr class='trow'>
						...
						<td style='mso-number-format:\"\@\";'>".$row->DESCP."</td>
						...
						<td style='mso-number-format:\"\@\";taxt-align:center;'>".$this->Convertdate(2,$row->INPDT)."</td>
		...
		if($i>0){
			$html = "
				<div id='table-fixed-ReportOutputtax_more' class='col-sm-12' style='height:100%;width:100%;overflow:auto;font-size:9pt;'>
					<table id='table-ReportOutputtax_more' style='background-color:white;' class='col-sm-12 display table table-bordered' cellspacing='0' width='calc(100% - 1px)'>
						<thead>
						<tr style='height:40px;'>
							<th colspan='13' style='font-size:12pt;border:0px;vertical-align;middle;text-align:center;'>รายงานภาษีขาย (ยื่นเพิ่มเติม)</th>
						</tr>
		...
		$report = "
			<div id='table-fixed-ReportOutputtax_more2' class='col-sm-12' style='height:100%;width:100%;overflow:auto;'>
				<table id='table-ReportOutputtax_more2' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='100%'>
					<thead>
						<tr>
							<th colspan='13' style='font-size:12pt;border:0px;text-align:center;'>รายงานภาษีขาย(ยื่นเพิ่มเติม)</th>
						</tr>
						<tr>
	...
	function pdf(){
		...
		$sql = "
				select TAXDT, TAXNO, CUSNAME, CONTNO, STRNO, DESCP, VATRT, NETAMT, VATAMT, TOTAMT, CANCEL, INPDT
				from #txsale
				order by CONTNO
		";//echo $sql; exit;
		$query = $this->db->query($sql);
		...
		$head = "
				<tr>
					...
					<th width='150px' 	style='border-top:0.1px solid black;border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>ชื่อ-นามสกุล<br>เลขที่สัญญา</th>
					...
					<th width='90px'	style='border-top:0.1px solid black;border-bottom:0.1px solid black;vertical-align:top;text-align:right;'>วันที่รันเพิ่มเติม</th>
			...
			foreach($query->result() as $row){	
				$html .= "
					<tr class='trow' seq=".$No.">
						<td width='40px' 	height='35px'	align='left' 	style='vertical-align:top;'>".$No++."</td>
						<td width='80px' 	height='35px'	align='left'  	style='vertical-align:top;'>".$this->Convertdate(2,$row->TAXDT)."</td>
						<td width='80px' 	height='35px'	align='left' 	style='vertical-align:top;'>".$row->TAXNO."</td>
						<td width='150px' 	height='35px'	align='left' 	style='vertical-align:top;'>".$row->CUSNAME."<br>".$row->CONTNO."</td>
						<td width='120px' 	height='35px'	align='left' 	style='vertical-align:top;'>".$row->STRNO."<br></td>
						<td width='80px' 	height='35px'	align='left' 	style='vertical-align:top;'>".$row->DESCP."</td>
						<td width='70px' 	height='35px'	align='center' 	style='vertical-align:top;'>".number_format($row->VATRT)."</td>
						<td width='100px' 	height='35px'	align='right' 	style='vertical-align:top;'>".number_format($row->NETAMT,2)."</td>
						<td width='100px' 	height='35px'	align='right' 	style='vertical-align:top;'>".number_format($row->VATAMT,2)."</td>
						<td width='100px' 	height='35px'	align='right' 	style='vertical-align:top;'>".number_format($row->TOTAMT,2)."</td>
						<td width='90px' 	height='35px'	align='right' 	style='vertical-align:top;'>".$this->Convertdate(2,$row->TAXDT)."</td>
						<td width='50px'	height='35px'	align='center' 	style='vertical-align:top;'>".$row->CANCEL."</td>
					</tr>
				";	
			}
		}
			...
			foreach($query2->result() as $row){	
				...
				$html .= "
					<tr class='trow' style='background-color:#ebebeb;'>
						...
						<th height='25px' style='".$bordertop."".$borderbottom."border-right:0.1px solid black;' colspan='2'></th>
					</tr>
		...
		$header = "
			<table class='wf fs9' cellspacing='0' style='border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
				<tr>
					<th colspan='12' style='font-size:11pt;'>".$COMP_NM."<br>รายงานภาษีขาย(ยื่นเพิ่มเติม)</th>
				</tr>
				<tr>
					<td colspan='12' style='height:35px;text-align:center;'>".$LOCATNM." จากวันที่ วันที่ ".$tx[2]." ถึงวันที่ ".$tx[3]."</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan='10' align='left'>".$LOCADDR1." ".$LOCADDR2."</td>
				</tr>
				<tr>
					<td colspan='2' align='left'>เลขประจำตัวผู้เสียภาษี</td>
					<td colspan='5' align='left'>".$TAXID."</td>
