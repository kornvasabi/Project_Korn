...
//BEE+
class ReportDeletestock extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
				...
				<div class='col-sm-12 col-xs-12' style='height:100%;overflow:auto;font-size:10.5pt;'>					
					<div class='row' style='height:90%;'>
						<div class='col-sm-12 col-xs-12' style='background-color:#0067a5;border:5px solid white;height:75px;text-align:center;font-size:12pt;color:white;font-weight:bold;'>	
							<br>รายงานตรวจสอบการลบรถในสต็อก<br>
						</div>
		...
		$html.= "<script src='".base_url('public/js/SYS02/ReportDeletestock.js')."'></script>";
		echo $html;
	}
	...
	function search(){
		...
		if($LOCAT1 != ""){
			$cond .= " AND (A.RVLOCAT LIKE '".$LOCAT1."%')";
			$rpcond .= "  สถานที่รับรถ ".$LOCAT1;
		}
		
		if($GCOCE1 != ""){
			$cond .= " AND (A.GCODE LIKE '".$GCOCE1."%') ";
		}
		
		if($TYPE1 != ""){
			$cond .= " AND (A.TYPE LIKE '".$TYPE1."%')";
		}
		
		if($BAAB1 != ""){
			$cond .= " AND (A.BAAB LIKE '".$BAAB1."%')";
		}
		
		if($MODEL1 != ""){
			$cond .= " AND (A.MODEL LIKE '".$MODEL1."%') ";
		}
		
		if($COLOR1 != ""){
			$cond .= " AND (A.COLOR LIKE '".$COLOR1."%') ";
		}
		
		if($stat != ""){
			$cond .= " AND (A.STAT LIKE '".$stat."%')";
		}
		...
		$sql = "
				...
				INTO #MAIN
				FROM(
					SELECT A.RECVNO, A.RECVDT, convert(char,A.RECVDT,112) as RECVDTS, convert(char,A.CANCELDT,112) as CANCELDT, 
					C.USERNAME as CANNAME, D.NAME as RECVNAME, B.USERNAME as USERNAME, A.RVLOCAT, A.STAT, A.GCODE, A.TYPE, A.MODEL, 
					A.BAAB, A.COLOR, A.CC, A.STRNO, A.ENGNO
					FROM {$this->MAuth->getdb('CANINVTR')} A, {$this->MAuth->getdb('PASSWRD')} B, {$this->MAuth->getdb('PASSWRD')} C, {$this->MAuth->getdb('OFFICER')} D 
					WHERE A.USERID=B.USERID AND A.CANCELID=C.USERID AND A.RVCODE=D.CODE 
					AND (A.RECVDT BETWEEN '".$FRMDATE."' AND '".$TODATE."') ".$cond." 
				)MAIN
		";//echo $sql; 
		$query = $this->db->query($sql);
		
		$sql = "
				SELECT RECVNO, RECVDTS, CANCELDT, CANNAME, RECVNAME, USERNAME, RVLOCAT, STAT, GCODE, TYPE, MODEL, 
				BAAB, COLOR, CC, STRNO, ENGNO
				FROM #MAIN
				ORDER BY ".$orderby."
		";//echo $sql; exit;
		$query = $this->db->query($sql);
		...
		$head = "<tr style='height:25px;'>
				...
				<th style='vertical-align:top;'>เลขที่ใบรับ<br>ยี่ห้อ </th>
				<th style='vertical-align:top;'>วันที่รับ<br>รุ่น</th>
				<th style='vertical-align:top;'>ผู้รับรถ<br>แบบ</th>
				<th style='vertical-align:top;'>ผุ้บันทึกระบบ<br>สี</th>
				<th style='vertical-align:top;'>สถานที่รับรถ<br>ขนาด</th>
				<th style='vertical-align:top;'>สถานภาพ<br>เลขตัวถัง</th>
				<th style='vertical-align:top;'>ประเภทสินค้า<br>เลขเครื่อง</th>
				<th style='vertical-align:top;'>ผู้ยกเลิก<br>วันที่ยกเลิก</th>
				</tr>
		";
			...
			foreach($query->result() as $row){$i++;
				$html .= "
					<tr class='trow' seq=".$NRow.">
						...
						<td>".$row->RECVNO."<br>".$row->TYPE."</td>
						<td>".$this->Convertdate(2,$row->RECVDTS)."<br>".$row->MODEL."</td>
						<td>".$row->RECVNAME."<br>".$row->BAAB."</td>
						<td>".$row->USERNAME."<br>".$row->COLOR."</td>
						<td>".$row->RVLOCAT."<br>".number_format($row->CC)."</td>
						<td>".$row->STAT."<br>".$row->STRNO."</td>
						<td>".$row->GCODE."<br>".$row->ENGNO."</td>
						<td>".$row->CANNAME."<br>".$this->Convertdate(2,$row->CANCELDT)."</td>
					</tr>
				";	
			}
		}
		...
		$head2 = "<tr>
					...
					<th style='vertical-align:top;'>ผู้รับรถ</th>
					<th style='vertical-align:top;'>ผุ้บันทึกระบบ</th>
					<th style='vertical-align:top;'>สถานที่รับรถ</th>
					<th style='vertical-align:top;'>สถานภาพ</th>
					...
					<th style='vertical-align:top;'>ผู้ยกเลิก</th>
					<th style='vertical-align:top;'>วันที่ยกเลิก</th>
				</tr>
		";
			...
			foreach($query->result() as $row){
				$report .= "
					<tr class='trow'>
						...
						<td style='mso-number-format:\"\@\";'>".$row->RECVNAME."</td>
						<td style='mso-number-format:\"\@\";'>".$row->USERNAME."</td>
						...
						<td style='mso-number-format:\"\@\";'>".$row->CANNAME."</td>
						<td style='mso-number-format:\"\@\";'>".$this->Convertdate(2,$row->CANCELDT)."</td>
		...
		if($query2->row()){
			foreach($query2->result() as $row){
				$sumreport2 = "
					<tr class='trow'>
						<th style='mso-number-format:\"\@\";text-align:left;' colspan='17'>".$row->Total."</th>
					</tr>
				";	
			}
		}
		...
		if($i>0){
			$html = "
				<div id='table-fixed-ReportDeletestock' class='col-sm-12' style='height:100%;width:100%;overflow:auto;font-size:9pt;'>
					<table id='table-ReportDeletestock' style='background-color:white;' class='col-sm-12 display table table-bordered' cellspacing='0' width='calc(100% - 1px)'>
						<thead>
						<tr style='height:40px;'>
							<th colspan='8' style='font-size:12pt;border:0px;vertical-align;middle;text-align:center;'>รายงานตรวจสอบการลบรถในสต็อก</th>
						</tr>
						<tr style='height:25px;'>
							<td colspan='8' style='border-bottom:1px solid #ddd;vertical-align;middle;text-align:center;'>จากวันที่ ".$_REQUEST["FRMDATE"]." - ".$_REQUEST["TODATE"]." ".$rpcond."  ออกรายงาน ณ วันที่ ".$this->today('today')."</td>
						</tr>
						".$head."
						</thead>	
		...
		$report = "
			<div id='table-fixed-ReportDeletestock2' class='col-sm-12' style='height:100%;width:100%;overflow:auto;'>
				<table id='table-ReportDeletestock2' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='100%'>
					<thead>
						<tr>
							<th colspan='17' style='font-size:12pt;border:0px;text-align:center;'>รายงานตรวจสอบการลบรถในสต็อก</th>
						</tr>
						<tr>
	...
	function pdf(){
		...
		$head = "
				<tr>
					...
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>เลขที่ใบรับ<br>ยี่ห้อ </th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>วันที่รับ<br>รุ่น</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>ผู้รับรถ<br>แบบ</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>ผุ้บันทึกระบบ<br>สี</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>สถานที่รับ<br>ขนาด</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>สถานภาพ<br>เลขตัวถัง</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>ประเภทสินค้า<br>เลขเครื่อง</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>ผู้ยกเลิก<br>วันที่ยกเลิก</th>
				</tr>
		";
			...
			foreach($query->result() as $row){	
				$html .= "
					<tr class='trow' seq=".$No.">
						...
						<td style='width:80px;'>".$row->RECVNO."<br>".$row->TYPE."</td>
						<td style='width:80px;'>".$this->Convertdate(2,$row->RECVDTS)."<br>".$row->MODEL."</td>
						<td style='width:120px;'>".$row->RECVNAME."<br>".$row->BAAB."</td>
						<td style='width:120px;'>".$row->USERNAME."<br>".$row->COLOR."</td>
						<td style='width:60px;'>".$row->RVLOCAT."<br>".number_format($row->CC)."</td>
						<td style='width:140px;'>".$row->STAT."<br>".$row->STRNO."</td>
						<td style='width:140px;'>".$row->GCODE."<br>".$row->ENGNO."</td>
						<td style='width:120px;'>".$row->CANNAME."<br>".$this->Convertdate(2,$row->CANCELDT)."</td>
					</tr>
				";	
			}
		}
		...
		$content = "
			<table class='wf' style='font-size:8pt;height:700px;border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
				<tbody>
					<tr>
						<th colspan='9' style='font-size:10pt;'>รายงานตรวจสอบการลบรถในสต็อก</th>
					</tr>
					<tr>
						<td colspan='9' style='font-size:9pt;height:35px;border-bottom:0.1px solid black;text-align:center;'>จากวันที่ ".$tx[6]." - ".$tx[7]." ".$rpcond."</td>
					</tr>
					".$head."
					".$html."
				</tbody>
