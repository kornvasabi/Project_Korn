...
********************************************************/
class Carsinstockcansale extends MY_Controller {
	private $sess = array(); 
	...
	function index(){
			...
			<div class='k_tab1' name='home' locat='{$this->sess['branch']}' cin='{$claim['m_insert']}' cup='{$claim['m_update']}' cdel='{$claim['m_delete']}' clev='{$claim['level']}' today='".$this->today('today')."' style='height:calc(100vh - 132px);overflow:auto;background-color:white;'>
				<div class='col-sm-12'>
					...
					<div class='col-sm-2'>	
						<div class='form-group'>
							<br>
							<div style='text-align:center;font-size:14pt;'>***ไม่รวมรถจอง***</div>
						</div>
					</div>
					<div class=' col-sm-2'>	
						<div class='form-group'>
		...
		";
		$html .="<script src='".base_url('public/js/SYS02/Carsinstockcansale.js')."'></script>";
		echo $html;
	}
	function Search(){
		$response = array();
		...
		if($TDATE == ""){
			$response["error"] = true;
			$response["msg"] = "กรุณาระบุวันที่ก่อนที่จะค้นหาครับ";
		...
		$sql = "
			...
			from {$this->MAuth->getdb('INVTRAN')} where (CURSTAT <> 'Y' or CURSTAT is null) 
			and (RECVDT <= '".$TDATE."') and (RESVDT > '".$TDATE."' or RESVDT is null) 
			and (SDATE > '".$TDATE."' or SDATE is null) ".$cond."
		...
		$html = "
			<table id='table-stockcarsale' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='99.99%' border=1 style='font-size:8pt;'>
