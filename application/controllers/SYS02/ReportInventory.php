...
class ReportInventory extends MY_Controller {
	...
	function index(){
		...
		$html.= "<script src='".base_url('public/js/SYS02/ReportInventory.js')."'></script>";
		echo $html;
	}
	...
	function search(){
		...
		$sql = "
				...
				from(
					...
					V.INVNO, convert(char,V.INVDT,112) as INVDT, T.NETCOST, T.CRVAT, T.TOTCOST, T.TADDCOST, T.BONUS, T.GCODE  
					...
					GROUP BY  V.RECVNO, V.RECVDT, V.INVNO, V.INVDT, V.TAXNO, V.TAXDT, T.GCODE, T.MOVENO, T.MOVEDT, T.TYPE, T.MODEL,
					T.BAAB, T.COLOR, T.CC, T.STRNO, T.ENGNO, T.KEYNO, T.STAT, T.RVLOCAT, T.CRLOCAT, T.MILERT, T.NETCOST, T.CRVAT,
					T.TOTCOST, T.TADDCOST, T.BONUS, E.STRNO, E.MOVEDT 
		...
		$sql = "
				select MODEL, BAAB, COLOR, STRNO, ENGNO, RECVNO, RECVDTS, CRLOCAT, INVNO, INVDT, NETCOST, CRVAT, TOTCOST, TADDCOST, BONUS
				from #main 
		...
		$sql = "
				select 'รวมทั้งหมด  '+convert(nvarchar,COUNT(STRNO))+' คัน' as Total, sum(NETCOST) as sumNETCOST, sum(CRVAT) as sumCRVAT, 
				sum(TOTCOST) as sumTOTCOST, sum(TADDCOST) as sumTADDCOST, sum(BONUS) as sumBONUS
				from #main
		";//echo $sql; exit;
		...
		$head = "<tr style='height:25px;'>
				...
				<th style='vertical-align:top;'>รุ่น<br>เลขที่ใบส่งของ</th>
				<th style='vertical-align:top;'>แบบ<br>วันที่ใบส่งของ</th>
				<th style='vertical-align:top;'>สี<br>เลขที่ใบรับ</th>
				<th style='vertical-align:top;'>เลขตัวถัง<br>วันที่รับ</th> 
				<th style='vertical-align:top;'>เลขเครื่อง<br>สาขาที่เก็บ</th>
				<th style='vertical-align:top;text-align:right;'>ราคาทุน<br>ต้นทุนซ่อม</th>
				<th style='vertical-align:top;text-align:right;'>ภาษีทุน<br>โบนัสพิเศษ</th>
			...
			foreach($query->result() as $row){$i++;
				$html .= "
					<tr class='trow' seq=".$NRow.">
						...
						<td>".$row->MODEL."<br>".$row->INVNO."</td>
						<td>".$row->BAAB."<br>".$this->Convertdate(2,$row->INVDT)."</td>
						<td>".$row->COLOR."<br>".$row->RECVNO."</td>
						<td>".$row->STRNO."<br>".$this->Convertdate(2,$row->RECVDTS)."</td>
						<td>".$row->ENGNO."<br>".$row->CRLOCAT."</td>
						<td align='right'>".number_format($row->NETCOST,2)."<br>".number_format($row->TADDCOST,2)."</td>
						<td align='right'>".number_format($row->CRVAT,2)."<br>".number_format($row->BONUS,2)."</td>
		...
		$head2 = "<tr>
					...
					<th style='vertical-align:top;'>เลขที่ใบส่งของ</th>
					<th style='vertical-align:top;'>วันที่ใบส่งของ</th>
					<th style='vertical-align:top;text-align:right;'>ราคาทุน</th>
					<th style='vertical-align:top;text-align:right;'>ภาษีทุน</th>
					<th style='vertical-align:top;text-align:right;'>รวมราคาทุน</th>
					<th style='vertical-align:top;text-align:right;'>ต้นทุนซ่อม</th>
					<th style='vertical-align:top;text-align:right;'>โบนัสพิเศษ</th>
				</tr>
		";
			...
			foreach($query->result() as $row){
				$report .= "
					<tr class='trow'>
						...
						<td style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->NETCOST,2)."</td>
						<td style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->CRVAT,2)."</td>
						<td style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->TOTCOST,2)."</td>
						<td style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->TADDCOST,2)."</td>
						<td style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->BONUS,2)."</td>
					</tr>
				";	
			}
		}
			...
			foreach($query2->result() as $row){
				$sumreport = "
					<tr style='height:25px;'>
						<th colspan='5' style='border-right:1px solid #ddd;border-left:0px;border-bottom:0px;border-top:0px;vertical-align:middle;text-align:left;'>".$row->Total."</th>
						<th style='border-right:1px solid #ddd;border-left:0px;border-bottom:0px;border-top:0px;vertical-align:middle;text-align:right;'>".number_format($row->sumNETCOST)."<br>".number_format($row->sumTADDCOST)."</th>
						<th style='border-right:1px solid #ddd;border-left:0px;border-bottom:0px;border-top:0px;vertical-align:middle;text-align:right;'>".number_format($row->sumCRVAT)."<br>".number_format($row->sumBONUS)."</th>
						<th style='border:0px;text-align:right;'>".number_format($row->sumTOTCOST)."</th>
					</tr>
				";	
			}
		}
		...
		if($query2->row()){
			foreach($query2->result() as $row){
				$sumreport2 = "
					<tr class='trow'>
						<th style='mso-number-format:\"\@\";text-align:left;' colspan='11'>".$row->Total."</th>
						...
						<th style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->sumTADDCOST,2)."</th>
						<th style='mso-number-format:\"\#\,\#\#0.00\";text-align:right;'>".number_format($row->sumBONUS,2)."</th>
					</tr>
				";	
			}
		}
		...
		if($i>0){
			$html = "
				<div id='table-fixed-ReportInventory' class='col-sm-12' style='height:100%;width:100%;overflow:auto;font-size:9pt;'>
					<table id='table-ReportInventory' style='background-color:white;' class='col-sm-12 display table table-bordered' cellspacing='0' width='calc(100% - 1px)'>
						<thead>
						<tr style='height:40px;'>
							<th colspan='8' style='font-size:12pt;border:0px;vertical-align;middle;text-align:center;'>รายงานสินค้าคงเหลือ (รถ)</th>
						</tr>
						<tr style='height:25px;'>
							<td colspan='8' style='border-bottom:1px solid #ddd;vertical-align;middle;text-align:center;'>คงเหลือ ณ วันที่  ".$_REQUEST["FRMDATE"]." ".$rpcond."  ออกรายงาน ณ วันที่ ".$this->today('today')."</td>
						</tr>
						".$head."
						</thead>	
		...
		$report = "
			<div id='table-fixed-ReportInventory2' class='col-sm-12' style='height:100%;width:100%;overflow:auto;'>
				<table id='table-ReportInventory2' class='col-sm-12 display table table-striped table-bordered' cellspacing='0' width='100%'>
					<thead>
						<tr>
							<th colspan='16' style='font-size:12pt;border:0px;text-align:center;'>รายงานสินค้าคงเหลือ (รถ)</th>
						</tr>
						<tr>
							<td colspan='16' style='border:0px;text-align:center;'>คงเหลือ ณ วันที่  ".$_REQUEST["FRMDATE"]." ".$rpcond."  ออกรายงาน ณ วันที่ ".$this->today('today')."</td>
						</tr>
						".$head2."
					</thead>	
					<tbody>
	...
	function pdf(){
		...
		$head = "
				<tr>
					...
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>รุ่น<br>เลขที่ใบส่งของ</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>แบบ<br>วันที่ใบส่งของ</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>สี<br>เลขที่ใบรับ</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>เลขตัวถัง<br>วันที่รับ</th> 
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:left;'>เลขเครื่อง<br>สาขาที่เก็บ</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:right;'>ราคาทุน<br>ต้นทุนซ่อม</th>
					<th style='border-bottom:0.1px solid black;vertical-align:top;text-align:right;'>ภาษีทุน<br>โบนัสพิเศษ</th>
			...
			foreach($query->result() as $row){	
				$html .= "
					<tr class='trow' seq=".$No.">
						...
						<td style='width:170px;'>".$row->MODEL."<br>".$row->INVNO."</td>
						<td style='width:130px;'>".$row->BAAB."<br>".$this->Convertdate(2,$row->INVDT)."</td>
						<td style='width:170px;'>".$row->COLOR."<br>".$row->RECVNO."</td>
						<td style='width:150px;'>".$row->STRNO."<br>".$this->Convertdate(2,$row->RECVDTS)."</td>
						<td style='width:150px;'>".$row->ENGNO."<br>".$row->CRLOCAT."</td>
						<td style='width:110px;' align='right'>".number_format($row->NETCOST,2)."<br>".number_format($row->TADDCOST,2)."</td>
						<td style='width:110px;' align='right'>".number_format($row->CRVAT,2)."<br>".number_format($row->BONUS,2)."</td>
						<td style='width:110px;' align='right'>".number_format($row->TOTCOST,2)."</td>
					</tr>
				";	
			}
		}
			...
			foreach($query2->result() as $row){	
				$html .= "
					<tr class='trow bor' style='background-color:#ebebeb;'>
						...
						<th align='right'>".number_format($row->sumNETCOST,2)."<br>".number_format($row->sumTADDCOST,2)."</th>
						<th align='right'>".number_format($row->sumCRVAT,2)."<br>".number_format($row->sumBONUS,2)."</th>
						<th align='right'>".number_format($row->sumTOTCOST,2)."</td>
					</tr>
		...
		$content = "
			<table class='wf' style='font-size:8.5pt;height:700px;border-collapse:collapse;line-height:23px;overflow:wrap;vertical-align:text-top;'>
				<tbody>
					<tr>
						<th colspan='11' style='font-size:10pt;'>รายงานสินค้าคงเหลือ (รถ)</th>
					</tr>
					<tr>
