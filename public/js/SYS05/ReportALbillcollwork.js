...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS05/ReportALbillcollwork/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานผลงานพนักงานเก็บเงิน',
				content: data.html,
			...
			$("#table-ALbillcollwork-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานผลงานพนักงานเก็บเงิน "+data.reporttoday); 
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS05/ReportALbillcollwork/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS05/ReportALbillcollwork/pdf?condpdf='+data[0];
