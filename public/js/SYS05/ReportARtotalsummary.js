...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS05/ReportARtotalsummary/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานลูกหนี้คงเหลือตามสาขา',
				content: data.html,
			...
			$("#print-ARtotalsummary").hover(function() {
				document.getElementById("print-ARtotalsummary").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-ARtotalsummary").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-ARtotalsummary-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-ARtotalsummary-excel").hover(function() {
				document.getElementById("table-ARtotalsummary-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-ARtotalsummary-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-ARtotalsummary-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานลูกหนี้คงเหลือตามสาขา "+data.reporttoday); 
			});
			
			$('#print-ARtotalsummary').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS05/ReportARtotalsummary/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS05/ReportARtotalsummary/pdf?condpdf='+data[0];
