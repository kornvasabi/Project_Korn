...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS05/ReportARkangpay_xy/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานลูกหนี้ค้างชำระ x งวด - y งวด',
				content: data.html,
			...
			$("#print-ARkangpay_xy").hover(function() {
				document.getElementById("print-ARkangpay_xy").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-ARkangpay_xy").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-ARkangpay_xy-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-ARkangpay_xy-excel").hover(function() {
				document.getElementById("table-ARkangpay_xy-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-ARkangpay_xy-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-ARkangpay_xy-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานลูกหนี้ค้างชำระ x งวด - y งวด "+data.reporttoday); 
			});
			
			$('#print-ARkangpay_xy').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS05/ReportARkangpay_xy/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS05/ReportARkangpay_xy/pdf?condpdf='+data[0];
