...
function search(){
	...
	dataToPost.EXPFRM1 		= $('#EXPFRM1').val();
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS05/ReportARkangpay_x/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานลูกหนี้ค้างชำระ x งวดแรก',
				content: data.html,
			...
			fn_datatables('table-ReportARkangpay_x',1,320);
			
			$('.data-export').prepend('<img id="print-ARkangpay_x" src="../public/images/print-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#print-ARkangpay_x").hover(function() {
				document.getElementById("print-ARkangpay_x").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-ARkangpay_x").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-ARkangpay_x-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-ARkangpay_x-excel").hover(function() {
				document.getElementById("table-ARkangpay_x-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-ARkangpay_x-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-ARkangpay_x-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานลูกหนี้ค้างชำระ x งวดแรก "+data.reporttoday); 
			});
			
			$('#print-ARkangpay_x').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS05/ReportARkangpay_x/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS05/ReportARkangpay_x/pdf?condpdf='+data[0];
