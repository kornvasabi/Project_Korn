...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_bk').attr('disabled',false);	
	}else{
		$('#add_bk').attr('disabled',true);	
	}
});

var jdsearch_bk = null;
$('#search_bk').click(function(){
	search();
});
...
function search(){
	...
	jdsearch_bk = $.ajax({
		...
		beforeSend: function(){ if(jdsearch_bk !== null){ jdsearch_bk.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		...
		$.ajax({
			url:'../setup/Company/BankGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
