...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_gar').attr('disabled',false);	
	}else{
		$('#add_gar').attr('disabled',true);	
	}
});

var jdsearch_gar = null;
$('#search_gar').click(function(){
	search();
});
...
function search(){
	...
	dataToPost.garname = $('#garname').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_gar = $.ajax({
		url: '../setup/Company/GarSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setGarResult').html(data.html);
			afterSearch();
			jdsearch_gar = null;
...
function afterSearch(){
	...
	$('.getit').click(function(){
		...
		$.ajax({
			url:'../setup/Company/GarGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.GARNAME = $('#t2GARNAME').val();
					dataToPost.GARADDR1 = $('#t2GARADDR1').val();
					dataToPost.GARADDR2 = $('#t2GARADDR2').val();
					dataToPost.GARTELP = $('#t2GARTELP').val();
					dataToPost.MEMO1 = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Company/GarSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
