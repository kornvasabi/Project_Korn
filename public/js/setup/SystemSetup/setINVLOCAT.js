...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_invlocat').attr('disabled',false);	
	}else{
		$('#add_invlocat').attr('disabled',true);	
	}
});

var jdsearch_invlocat = null;
$('#search_invlocat').click(function(){
	search();
});

function search(){
	dataToPost = new Object();
	dataToPost.LOCATCD = $('#LOCATCD').val();
	dataToPost.LOCATNM = $('#LOCATNM').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_invlocat = $.ajax({
		url: '../setup/SystemSetup/SETINVLOCATSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setInvlocatResult').html(data.html);
			afterSearch();
			jdsearch_invlocat = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_invlocat !== null){ jdsearch_invlocat.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		...
		$.ajax({
			url: '../setup/SystemSetup/SETINVLOCATGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.LOCATCD 	= $('#t2LOCATCD').val();
					dataToPost.LOCATNM 	= $('#t2LOCATNM').val();
					dataToPost.SHORTL  	= $('#t2SHORTL').val();
					dataToPost.LOCADDR1 = $('#t2LOCADDR1').val();
					dataToPost.LOCADDR2 = $('#t2LOCADDR2').val();
					dataToPost.FLSALE 	= ($('#t2FLSALE').is(":checked")?"T":"F");
					dataToPost.MEMO1 	= $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url: '../setup/SystemSetup/SETINVLOCATSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$('#loadding').fadeIn(200);
					$.ajax({
						url: '../setup/SystemSetup/SETINVLOCATDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
