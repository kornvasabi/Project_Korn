...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_depmast').attr('disabled',false);	
	}else{
		$('#add_depmast').attr('disabled',true);	
	}
});

var jdsearch_depmast = null;
$('#search_depmast').click(function(){
	search();
});

function search(){
	dataToPost = new Object();
	dataToPost.DEPCODE = $('#DEPCODE').val();
	dataToPost.DEPNAME = $('#DEPNAME').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_depmast = $.ajax({
		url: '../setup/SystemSetup/SETDEPMASTSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setDepmastResult').html(data.html);
			afterSearch();
			jdsearch_depmast = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_depmast !== null){ jdsearch_depmast.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.DEPCODE = $(this).attr('DEPCODE');
		...
		$.ajax({
			url: '../setup/SystemSetup/SETDEPMASTGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.DEPNAME 	= $('#t2DEPNAME').val();
					dataToPost.MEMO1 	= $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url: '../setup/SystemSetup/SETDEPMASTSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$('#loadding').fadeIn(200);
					$.ajax({
						url: '../setup/SystemSetup/SETDEPMASTDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
