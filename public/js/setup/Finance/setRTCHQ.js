...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_RT').attr('disabled',false);	
	}else{
		$('#add_RT').attr('disabled',true);	
	}
});

var jdsearch_RT = null;
$('#search_RT').click(function(){
	search();
});

function search(){
	dataToPost = new Object();
	dataToPost.RTCODE = $('#RTCODE').val();
	dataToPost.RTDESC = $('#RTDESC').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_RT = $.ajax({
		url: '../setup/Finance/RTCHQSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setRTResult').html(data.html);
			afterSearch();
			jdsearch_RT = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_RT !== null){ jdsearch_RT.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.RTCODE = $(this).attr('RTCODE');
		...
		$.ajax({
			url:'../setup/Finance/RTCHQGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.RTCODE = $('#t2RTCODE').val();
					dataToPost.RTNAME = $('#t2RTNAME').val();
					dataToPost.MEMO1 = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Finance/RTCHQSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
