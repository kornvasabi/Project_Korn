...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_paytyp').attr('disabled',false);	
	}else{
		$('#add_paytyp').attr('disabled',true);	
	}
});

var jdsearch_paytyp = null;
$('#search_paytyp').click(function(){
	search();
});
...
function search(){
	...
	$('#loadding').fadeIn(200);
	jdsearch_paytyp = $.ajax({
		url: '../setup/Finance/PAYTYPSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setPAYTYPResult').html(data.html);
			afterSearch();
			jdsearch_paytyp = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_paytyp !== null){ jdsearch_paytyp.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		...
		$.ajax({
			url:'../setup/Finance/PAYTYPGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.RLBILL = ($('#t2RLBILL').is(":checked")?"Y":"N");
					dataToPost.MEMO1 = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Finance/PAYTYPSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
