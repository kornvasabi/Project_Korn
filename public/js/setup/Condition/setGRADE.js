...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_grade').attr('disabled',false);	
	}else{
		$('#add_grade').attr('disabled',true);	
	}
});

var jdsearch_grade = null;
$('#search_grade').click(function(){
	search();
});
...
function search(){
	...
	$('#loadding').fadeIn(200);
	jdsearch_grade = $.ajax({
		url: '../setup/Condition/SETGRADESearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setGradeResult').html(data.html);
			afterSearch();
			jdsearch_grade = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_grade !== null){ jdsearch_grade.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		...
		$.ajax({
			url:'../setup/Condition/SETGRADEGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.GRDCAL = $('#t2GRDCAL').val();
					...
					$.ajax({
						url:'../setup/Condition/SETGRADESave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$.ajax({
						url:'../setup/Condition/SETGRADEDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
