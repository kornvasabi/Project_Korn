...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_argroup').attr('disabled',false);	
	}else{
		$('#add_argroup').attr('disabled',true);	
	}
});

var jdsearch_argroup = null;
$('#search_argroup').click(function(){
	search();
});

function search(){
	dataToPost = new Object();
	dataToPost.ARGCOD = $('#ARGCOD').val();
	dataToPost.ARGDES = $('#ARGDES').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_argroup = $.ajax({
		url: '../setup/Condition/ARGROUPSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setArgroupResult').html(data.html);
			afterSearch();
			jdsearch_argroup = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_argroup !== null){ jdsearch_argroup.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.ARGCOD = $(this).attr('ARGCOD');
		...
		$.ajax({
			url:'../setup/Condition/ARGROUPGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.ARGCOD = $('#t2ARGCOD').val();
					dataToPost.ARGDES = $('#t2ARGDES').val();
					dataToPost.MEMO1  = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Condition/ARGROUPSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$.ajax({
						url:'../setup/Condition/ARGROUPDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
