...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_typhold').attr('disabled',false);	
	}else{
		$('#add_typhold').attr('disabled',true);	
	}
});

var jdsearch_typhold = null;
$('#search_typhold').click(function(){
	search();
});
...
function search(){
	...
	$('#loadding').fadeIn(200);
	jdsearch_typhold = $.ajax({
		url: '../setup/Condition/TYPHOLDSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setTypholdResult').html(data.html);
			afterSearch();
			jdsearch_typhold = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_typhold !== null){ jdsearch_typhold.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.HOLDCOD = $(this).attr('HOLDCOD');
		...
		$.ajax({
			url:'../setup/Condition/TYPHOLDGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.HOLDCOD = $('#t2HOLDCOD').val();
					dataToPost.HOLDESC = $('#t2HOLDESC').val();
					dataToPost.MEMO1   = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Condition/TYPHOLDSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$.ajax({
						url:'../setup/Condition/TYPHOLDDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
