...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_argar').attr('disabled',false);	
	}else{
		$('#add_argar').attr('disabled',true);	
	}
});

var jdsearch_argar = null;
$('#search_argar').click(function(){
	search();
});

function search(){
	dataToPost = new Object();
	dataToPost.GARCODE = $('#GARCODE').val();
	dataToPost.GARDESC = $('#GARDESC').val();
	
	$('#loadding').fadeIn(200);
	jdsearch_argar = $.ajax({
		url: '../setup/Condition/SETARGARSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setArgarResult').html(data.html);
			afterSearch();
			jdsearch_argar = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_argar !== null){ jdsearch_argar.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.GARCODE = $(this).attr('GARCODE');
		...
		$.ajax({
			url:'../setup/Condition/SETARGARGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.GARCODE = $('#t2GARCODE').val();
					dataToPost.GARDESC = $('#t2GARDESC').val();
					dataToPost.MEMO1 = $('#t2MEMO1').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Condition/SETARGARSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$.ajax({
						url:'../setup/Condition/SETARGARDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
