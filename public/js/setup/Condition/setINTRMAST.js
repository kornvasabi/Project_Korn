...
$(function(){
	if($('.tab1[name="home"]').attr('cin') == 'T'){
		$('#add_intrmast').attr('disabled',false);	
	}else{
		$('#add_intrmast').attr('disabled',true);	
	}
});

var jdsearch_intrmast = null;
$('#search_intrmast').click(function(){
	search();
});
...
function search(){
	...
	$('#loadding').fadeIn(200);
	jdsearch_intrmast = $.ajax({
		url: '../setup/Condition/INTRMASTSearch',
		data:dataToPost,
		type: 'POST',
		dataType: 'json',
		success: function(data){
			$('#setIntrmastResult').html(data.html);
			afterSearch();
			jdsearch_intrmast = null;
			$('#loadding').fadeOut(200);
		},
		beforeSend: function(){ if(jdsearch_intrmast !== null){ jdsearch_intrmast.abort(); } },
...
function afterSearch(){
	...
	$('.getit').click(function(){
		dataToPost = new Object();
		dataToPost.FRMDATE = $(this).attr('FRMDATE');
		dataToPost.TODATE = $(this).attr('TODATE');
		...
		$.ajax({
			url:'../setup/Condition/INTRMASTGetFormAE',
			data:dataToPost,
			type:'POST',
			dataType:'json',
			success:function(data){
				...
				$('#t2FRMDATE').attr('disabled',true);
				$('#t2TODATE').attr('disabled',true);
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					dataToPost.FRMDATE 	= $('#t2FRMDATE').val();
					dataToPost.TODATE 	= $('#t2TODATE').val();
					dataToPost.INTR 	= $('#t2INTR').val();
					
					dataToPost.action = $('#tab2save').attr('action');
					
					$.ajax({
						url:'../setup/Condition/INTRMASTSave',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
		...
		Lobibox.confirm({
			...
			callback: function(lobibox, type){
				...
				if (type === 'ok'){
					...
					$.ajax({
						url:'../setup/Condition/INTRMASTDel',
						data:dataToPost,
						type:'POST',
						dataType:'json',
						success:function(data){
