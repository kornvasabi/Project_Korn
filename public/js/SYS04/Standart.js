				...
				shown: function($this){
					...
					function redraw(){
						...
						$(".stddetail").click(function(){
							...
							$('#loadding').show();
							JDstddetail = $.ajax({
								url:'../SYS04/Standart/searchDetail',
								data: dataToPost,
								type: 'POST',
								dataType: 'json',
								success: function(data){
					...
					$("#excelstd").click(function(){	
						var d = new Date();
						tableToExcel_Export(data.excel,"ข้อมูล std","standart_"+(d.getTime())); 
						//tableToExcel_Export(data.html,"ข้อมูล std","standart_"+(d.getTime())); 
					});
...
function edit($btn){
	Lobibox.confirm({
		...
		closeOnEsc: false,
		closeButton: false,
		msg: "คุณต้องการแก้ไข Standart หรือไม่",
		buttons: {
			ok : {
...
function fnload($thisForm){
	...
	*/
	...
	$("#btnAddDwn").click(function(){
		loadAddDwn(null); 	// เพิ่มข้อมูล  Standart การดาวน์รถ  ใหม่
	});
	...
	function activeDatatables($stdfa){ // หลังจากเพิ่มข้อมูลใน datatables ให้ทำอะไรต่อ
		...
		$(".deleteDwn").click(function(){
			...
			Lobibox.confirm({
				...
				closeOnEsc: false,
				closeButton: false,
				msg: "คุณต้องการลบข้อมูล Standart การดาวน์รถ<br>ในช่วงเงินดาวน์ "+$deleteDwn.attr("formdwns")+" - "+$deleteDwn.attr("formdwne")+" ใช่ป่าววว ?",
				buttons: {
					ok : {
						'class': 'btn btn-primary glyphicon glyphicon-ok',
				...
				callback: function(lobibox, type){
					if (type === 'ok'){
						$deleteDwn.parent().parent().each(function(){ // tr
							$stdfa.row($(this)).remove().draw( false ); //ลบข้อมูล Standart
						});
					}
				}
			});
		...
		$('.editDwn').click(function(){
			...
			loadAddDwn($(this)); // แก้ข้อมูล  Standart การดาวน์รถ  
		});
	}
	
	$("#btnAddFree").click(function(){
		loadAddFree(null); 	// เพิ่มข้อมูล  Standart การดาวน์รถ  ใหม่
	});
	...
	function loadAddFree($edit){
		...
		$('#loadding').show(0);
		JDbtnAddDwn = $.ajax({
			url:'../SYS04/Standart/formStdFREE',
			data: dataToPost,
			type: 'POST',
			dataType: 'json',
			success: function(data){
	...
	function activeDatatablesFree($stdfree){ // หลังจากเพิ่มข้อมูลใน datatables ให้ทำอะไรต่อ
		...
		$(".deleteFree").click(function(){
			...
			Lobibox.confirm({
				...
				closeOnEsc: false,
				closeButton: false,
				msg: "คุณต้องการลบข้อมูล  Standart ของแถม <br>ในช่วงเงินดาวน์ "+$deleteFree.attr("formnopays")+" - "+$deleteFree.attr("formnopaye")+" ใช่ป่าววว ?",
				buttons: {
					ok : {
						'class': 'btn btn-primary glyphicon glyphicon-ok',
						text: ' ยืนยัน, ลบข้อมูล',
				...
				callback: function(lobibox, type){
					if (type === 'ok'){
						$deleteFree.parent().parent().each(function(){ // tr
							$stdfree.row($(this)).remove().draw( false ); //ลบข้อมูล Standart
						});
					}
				}
			});
		...
		$('.editFree').click(function(){
			...
			loadAddFree($(this)); // แก้ข้อมูล  Standart การดาวน์รถ  
		});
	}
...
function FN_JD_BSSELECT($id,$thisSelected){
	...
	function getdata(){
		...
		JDselectSACTICOD2_Cache = $.ajax({
			url: '../SYS04/Standart/getSACTICOD2',
			data: dataToPost,
			type: "POST",
			dataType: "json",
			success: function(data){
