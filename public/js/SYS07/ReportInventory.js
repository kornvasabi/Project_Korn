...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS07/ReportInventory/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			...
			document.getElementById("table-fixed-ReportInventory").addEventListener("scroll", function(){
				var translate = "translate(0,"+(this.scrollTop - 7)+"px)";
				this.querySelector("thead").style.transform = translate;
				this.querySelector("thead").style.zIndex = 100;
			});
	
			$('#H_ReportInventory').prepend('<img id="print-Inventory" src="../public/images/print-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			...
			$('#H_ReportInventory').prepend('<img id="table-Inventory-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			...
			$("#table-Inventory-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานสินค้าคงเหลือ "+data.reporttoday); 
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS07/ReportInventory/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS07/ReportInventory/pdf?condpdf='+data[0];
