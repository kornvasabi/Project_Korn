...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS02/ReportInventorytocheck/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานสินค้าคงเหลือ (รถ) เพื่อเช็คสต๊อก',
				content: data.html,
			...
			fn_datatables('table-ReportInventorytocheck',1,290);
			
			$('.data-export').prepend('<img id="print-Inventorytocheck" src="../public/images/print-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#print-Inventorytocheck").hover(function(){
				document.getElementById("print-Inventorytocheck").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-Inventorytocheck").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-Inventorytocheck-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-Inventorytocheck-excel").hover(function() {
				document.getElementById("table-Inventorytocheck-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-Inventorytocheck-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-Inventorytocheck-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานสินค้าคงเหลือ (รถ)  เพื่อเช็คสต๊อก "+data.reporttoday); 
			});
			
			$('#print-Inventorytocheck').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS02/ReportInventorytocheck/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS02/ReportInventorytocheck/pdf?condpdf='+data[0];
