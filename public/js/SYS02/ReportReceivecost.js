...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS02/ReportReceivecost/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			$('#loadding').hide();	
			Lobibox.window({
				title: 'รายงานการรับสินค้าเพื่อดูราคาทุน',
				content: data.html,
			...
			fn_datatables('table-ReportReceivecost',1,295);
			
			$('.data-export').prepend('<img id="print-Receivecost" src="../public/images/print-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#print-Receivecost").hover(function() {
				document.getElementById("print-Receivecost").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-Receivecost").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-Receivecost-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-Receivecost-excel").hover(function() {
				document.getElementById("table-Receivecost-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-Receivecost-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-Receivecost-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานการรับสินค้าเพื่อดูราคาทุน "+data.reporttoday); 
			});
			
			$('#print-Receivecost').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS02/ReportReceivecost/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS02/ReportReceivecost/pdf?condpdf='+data[0];
