...
function search(){
	...
	$('#loadding').show();
	reportsearch = $.ajax({
		url: '../SYS02/ReportReceivebydatereceipt/search',
		data: dataToPost,
		Type: 'POST',
		dataType:'json',
		success: function(data){	
			...
			fn_datatables('table-ReportReceivebydatereceipt',1,290);
			
			$('.data-export').prepend('<img id="print-Receivebydatereceipt" src="../public/images/print-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#print-Receivebydatereceipt").hover(function(){
				document.getElementById("print-Receivebydatereceipt").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("print-Receivebydatereceipt").style.filter = "contrast(100%)";
			});
			
			$('.data-export').prepend('<img id="table-Receivebydatereceipt-excel" src="../public/images/excel-icon.png" style="width:30px;height:30px;cursor:pointer;filter: contrast(100%);">');
			$("#table-Receivebydatereceipt-excel").hover(function() {
				document.getElementById("table-Receivebydatereceipt-excel").style.filter = "contrast(70%)";
			}, function() {
				document.getElementById("table-Receivebydatereceipt-excel").style.filter = "contrast(100%)";
			});
			
			$("#table-Receivebydatereceipt-excel").click(function(){ 
				tableToExcel_Export(data.report,"sheet 1","รายงานตรวจสอบการรับสินค้า "+data.reporttoday); 
			});
			
			$('#print-Receivebydatereceipt').click(function(){
				printReport();
			});
...
function printReport(){
	...
	$.ajax({
		url: '../SYS02/ReportReceivebydatereceipt/conditiontopdf',
		data: dataToPost,
		type:'POST',
		dataType: 'json',
		success: function(data){
			...
			var url = baseUrl+'SYS02/ReportReceivebydatereceipt/pdf?condpdf='+data[0];
